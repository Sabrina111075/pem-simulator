...
with st.sidebar.expander("🌍 Environment & Grade / 地形環境", expanded=True):
    slope_deg = st.sidebar.slider("Road Slope / 行駛坡度 (%)", -15.0, 15.0, 2.41)
    cd_factor = st.sidebar.slider("Air Drag / 風阻係數 (Cd)", 0.1, 1.2, 0.45)
    rolling_res = st.sidebar.slider("Rolling / 滾動阻力", 0.01, 0.05, 0.01)
...
else: sys_status = "Flat Road / 平坦路面"

# 3. 主畫面
st.title("⚡ Electric Motorcycle Propulsion Simulator")
st.subheader("電動機車動力模擬系統 - 工程整合版")
st.caption(f"TAD-AGE Engine V4.5 | {tw_now.strftime('%Y-%m-%d %H:%M:%S')}")
st.markdown("---")
