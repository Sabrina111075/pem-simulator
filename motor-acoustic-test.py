...
# 🏭 B. 其他工業設備指引 (保留原本字典查表邏輯)
    ...
    if alert_level == "GREEN":
        ...
        st.warning(f"🟡 **預警診斷：{title_str}**\n\n**1. 聲學診斷結果**：{diag_str}\n\n**2. 即時處置 SOP**：\n{sop_str}")
    else:
        guide_details_red = {
            "工業風扇 (Fan)": ("風扇葉片嚴重破損", "出現強烈高頻噪聲", "1. 即刻安排停機並切斷主電源。"),
            "工業馬達 (Motor)": ("馬達軸承嚴重磨損", "湧現強烈金屬磨損衝擊波", "1. 發布 Level 1 緊迫告警。")
        }
        title_str, diag_str, sop_str = guide_details_red.get(category, ("設備嚴重故障", "特徵值高度異常", "1. 立即停機檢修。"))
        st.error(f"🔴 **嚴重警告：{title_str}**\n\n**1. 聲學診斷結果**：{diag_str}\n\n**2. 即時處置 SOP**：\n{sop_str}")
    ...
    guide_details_red = {
        ...
        # 🚗 必須在這裡補上這段，轉子卡死/堵塞才會抓到！
        "電動車電池水冷泵浦 (EV Battery Cooling)": (
            "水冷泵浦轉子卡死 / 嚴重空蝕 (Critical Fault)",
            "偵測到強烈低頻電磁嗡鳴聲（卡死）或強烈爆裂聲（空蝕），電池組已失去水冷散熱能力",
            "1. 請尋找安全地點靠邊停車，切換至 P 檔並關閉車輛電源。\n2. 切勿嘗試進行高功率快充，以防電池過熱啟動保護機制或引發熱失控風險。\n3. 請聯繫道路救援或原廠拖吊服務，安排回廠檢測與更換水冷泵浦總成。"
        )
    }
