...
def calculate_geometry_metrics(chip_nfs, futures_diff):
    ...
    fts = (0.20 * 0.5) + (0.20 * (1 - curvature)) + (0.15 * 0.4) + (0.15 * chip_nfs) + (0.15 * 0.6) + (0.15 * (futures_diff / 100))
    trend_score = 100.0 * np.tanh(fts)
...
with col_title:
    ...
    st.caption("Market Manifold (E, V, A) × Pre-Market Force Field (NFS) × TimesFM / Chronos-2 Ensemble")
...
curvature, turning_risk, trend_score = calculate_geometry_metrics(chip_nfs, futures_diff)
...
with m2:
    st.metric(
        label="預測趨勢分數 (TrendScore)",
        value=f"{trend_score:+.1f}",
        delta="多頭主導 (Bullish)" if trend_score > 30 else "弱勢震盪 (Consolidation)"
    )
...
st.markdown("---")
st.caption("GeodesicX Simulation Platform | Powered by DMEC-GF Engine & Streamlit | 數據來源：臺灣證券交易所 (TWSE)")
    # 模擬 Softmax 週期分佈
