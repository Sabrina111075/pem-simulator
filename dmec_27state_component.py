...
# 上方 4 張指標卡片說明補強
    ...
    with c1:
        st.metric(
            label="當前 27 狀態碼 (S_t)",
            value=f"{state_tuple}",
            help="由三維狀態向量 S_t = (C, F, P) 構成，各位置分別代表：\n\n"
            "• 第 1 個數字 (C, 籌碼)：主力買賣意向與籌碼集中度\n"
            "• 第 2 個數字 (F, 資金)：主力淨資金流向與發動規模\n"
            "• 第 3 個數字 (P, 價差)：盤前試撮價差與開盤氣氛\n\n"
            "各數值定義：+1 (偏多/灌入) | 0 (中立/觀望) | -1 (偏空/流出)\n"
            "三維組合共計 3³ = 27 種幾何狀態碼。",
        )
    ...
    with c2:
        st.metric(
            ...
            help="解析狀態碼 (C, F, P) 三維組合之流場動態：\n\n"
            "【位置結構解析】\n"
            "• 第 1 位 (C)：預測大戶鎖碼與控盤力道\n"
            "• 第 2 位 (F)：預測資金推升/拉回之量能續航力\n"
            "• 第 3 位 (P)：預測開盤瞬間之動態衝擊與急拉/急甩機率\n\n"
            "【典型流場範例】\n"
            "• (1, 1, 1)：三維偏多共振，代表開高衝高且資金鎖碼強勁\n"
            "• (-1, -1, -1)：三維偏空共振，代表主力出貨且面臨賣壓擠壓\n"
            "• (1, 1, -1) 或 (0, 1, 1)：代表多空交織、洗盤或籌碼換手的非線性轉折流場。",
        )
