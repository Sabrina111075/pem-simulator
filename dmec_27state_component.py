...
# 上方 4 張指標卡片說明補強
    ...
    with c1:
        st.metric(
            label="當前 27 狀態碼 (S_t)",
            value=f"{state_tuple}",
            help="由三維狀態向量 S_t = (C, F, P) 構成，各數值代表：\n\n"
            "• [+1] 偏多：主力積極買超 / 資金淨注入 / 盤前開高溢價\n"
            "• [ 0] 中立：主力觀望平穩 / 資金持平 / 盤前平盤無價差\n"
            "• [-1] 偏空：主力調節賣超 / 資金淨流出 / 盤前開低折價\n\n"
            "位置順序：第 1 位籌碼 C、第 2 位資金 F、第 3 位價差 P。",
        )
    ...
    with c2:
        st.metric(
            ...
            help="解析狀態碼 (C, F, P) 三維數值涵義：\n\n"
            "【數值定義】\n"
            "• +1：動能偏多灌入（買盤鎖碼 / 推升資金 / 急拉衝擊）\n"
            "•  0：動能中性平衡（盤整觀望 / 資金平穩 / 無趨勢）\n"
            "• -1：動能偏空壓制（賣壓出貨 / 資金撤離 / 壓低衝擊）\n\n"
            "【位置對應】\n"
            "• 第 1 位 (C)：預測主力鎖碼與控盤力道\n"
            "• 第 2 位 (F)：預測資金推升/拉回之量能續航力\n"
            "• 第 3 位 (P)：預測開盤瞬間之動態衝擊與氣氛",
        )
