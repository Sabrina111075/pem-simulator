...
# 上方 4 張指標卡片說明補強
    ...
    with c1:
        st.metric(
            label="當前 27 狀態碼 (S_t)",
            value=f"{state_tuple}",
            help="三維狀態向量 S_t = (C, F, P) 各數值詳細定義：\n\n"
            "• 第 1 位 C (籌碼)：+1 買超鎖碼 | 0 中立觀望 | -1 賣超調節\n"
            "• 第 2 位 F (資金)：+1 淨資金流入 | 0 資金平穩 | -1 淨資金流出\n"
            "• 第 3 位 P (價差)：+1 盤前溢價(開高) | 0 平盤 | -1 盤前折價(開低)\n\n"
            "組合共計 3³ = 27 種幾何狀態碼。",
        )
    ...
    with c2:
        st.metric(
            ...
            help="【流場狀態名稱與幾何涵義】\n"
            "將 (C, F, P) 三維向量映射至雙曲幾何流場，用以標示當前市場的動態特徵類型：\n\n"
            "• ( 1,  1,  1) 極度多頭共振：主力鎖碼、資金大舉灌入且開高\n"
            "• (-1, -1, -1) 極度空頭擠壓：主力出貨、資金流出且開低\n"
            "• ( 0,  1,  1) 資金推升衝擊：籌碼中立但資金與價差強勢\n"
            "• ( 1,  0,  0) 籌碼暗中鎖碼：主力吸籌但資金與價差未發動\n"
            "• ( 0,  0,  0) 幾何平衡觀望：多空均衡無明顯趨勢\n\n"
            "【位置數值定義】\n"
            "• 第 1 位 C (籌碼)：+1 買超 | 0 觀望 | -1 賣超\n"
            "• 第 2 位 F (資金)：+1 流入 | 0 平穩 | -1 流出\n"
            "• 第 3 位 P (價差)：+1 溢價 | 0 平盤 | -1 折價",
        )
