...
def check_controller_compatibility(platform, sensor_type, voltage):
    ...
    if platform == "OD220":
        ...
        if voltage < 400:
            alerts.append("⚠️ 電壓警告：OD220 建議 400V 以上高壓[cite: 1]")
        recommendation = "💡 推薦對接：匯川技術 (Inovance)、英威騰 (INVT)[cite: 1]"
    elif platform in ["OD120", "OD140"]:
        if sensor_type == "Resolver":
            alerts.append("ℹ️ 成本建議：OD120/140 選用 Hall 或 Encoder 即可[cite: 1]")
        recommendation = "💡 推薦對接：安乃達 (Ananda)、天津松正 (Santroll)[cite: 1]"
            
    return alerts, recommendation

def simulate_motor_performance(v_bus, gear_ratio, tire_radius, weight, slope, platform_type):
    """維持原有穩定運行的動態物理模擬邏輯"""
    specs = {
...
# ==========================================
# 2. Streamlit UI 介面層 (完全保留原版比例)
# ==========================================

st.set_page_config(page_title="TAD-AGE 電車電機開發決策系統", layout="wide")

# 標題
st.header(f"🏢 {platform if 'platform' in locals() else 'OD120'} 電車電機開發決策系統平台")
...
p_peak, t_wheel, v_max, t_climb_req, t_motor_peak, rpm_limit = simulate_motor_performance(
    v_bus, gear_ratio, tire_radius, weight, slope, platform
)
...
# 下方主區塊：左圖右資料 (維持原版圖表大小)
col_main_left, col_main_right = st.columns([2.5, 1])

with col_main_left:
    st.subheader("📈 系統效率區間與作業特性曲線")
    rpm_range = np.linspace(0, rpm_limit * 1.2, 100)
    # 模擬恆扭矩與恆功率區
    torque_curve = [t_motor_peak if r < rpm_limit else t_motor_peak * rpm_limit / r for r in rpm_range]
    
    fig, ax = plt.subplots(figsize=(10, 5)) # 確保圖表夠大
...
with col_main_right:
    ...
    if alerts:
        ...
        st.success("✅ 控制器技術規格匹配成功[cite: 1]")
        
    st.info(rec_vendor)
    
    with st.expander("📝 獲取詢價郵件模板"):
        st.code(f"主旨：{platform} 電機控制器開發技術對接[cite: 1]\n內容：需支援 FOC、{sensor_type}、CAN 通訊...")

st.caption(f"最後更新：2026-05-05 | 框架版本：TAD-AGE v2.0 | 數據來源：電機控制器開發規劃[cite: 1]")