...
with st.sidebar.expander("🛠️ 控制器演算法與硬體配置", expanded=True):
    ...
    # 弱磁控制開關：影響高轉速表現
    enable_fw = st.toggle("弱磁控制 (Field Weakening)", value=True, help="進入恆功率區的關鍵演算法")[cite: 1]
    enable_regen = st.toggle("回生煞車 (Regenerative Braking)", value=False)[cite: 1]

    st.divider()
    
    st.subheader("硬體介面")
    # 根據選定平台動態推薦感測器[cite: 1]
...
# --- 5. 數據總結與建議 ---
st.info(f"💡 **目前配置總結**：\n"
        f"*   平台：{selected_platform} | 電壓：{spec['v']} | 冷卻：{spec['cooling']}[cite: 1]\n"
        f"*   控制器：FOC {'+ 弱磁' if enable_fw else ''} | 感測器：{selected_sensor}[cite: 1]\n"
        f"*   通訊：{', '.join(selected_comm)}[cite: 1]")

# 推薦供應商 (根據[cite: 1])
st.subheader("🏢 推薦對接供應商")
if selected_platform == "OD220":
    st.success("**首選夥伴**：匯川技術 (Inovance)、英威騰 (INVT)、精進電動 (JJE)[cite: 1]")
else:
    st.success("**首選夥伴**：安乃達 (Ananda)、天津松正 (Santroll)[cite: 1]")
