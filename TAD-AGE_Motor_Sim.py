...
c1.metric("平台峰值功率", f"{conf['p_peak']} kW")
c2.metric("最大輪端扭矩", f"{t_wheel_max:.1f} Nm")
c3.metric("當前爬坡需求扭矩", f"{t_req_wheel:.1f} Nm", 
          delta=f"電機負荷 {t_climb_req_motor:.1f} Nm", delta_color="inverse")
c4.metric("模擬環境極速", f"{v_max:.1f} km/h")
...
ax1.axhline(y=t_climb_req_motor, color='orange', linestyle='--', linewidth=2.5, label=f"Climb Req ({weight}kg)")
...
ax1.legend(lines1 + lines2, labels1 + labels2, loc='upper center', bbox_to_anchor=(0.5, 1.12), ncol=3)
    ...
    for i, v in enumerate(conf["vendors"]):
        with v_cols[i]:
            ...
            st.markdown(f"""<div class="vendor-card-body">
                <p style="color: #185abc; font-weight: 500;">📍 定位：{v['pos']}</p>
                <p style="color: #666; font-size: 14px;">🏷️ 核心：{v['tag']}</p>
            </div>""", unsafe_allow_html=True)
            st.button(f"🚀 選擇對接 {v['name'].split(' ')[0]}", key=f"v_btn_{i}")

with tabs[1]: # 系統 BOM (整合表格)
    st.subheader("🛠️ 系統 BOM 與預算成本整合表")
...
with tabs[2]: # 認證與熱管理
    ...
    with t_col2:
        ...
        st.success(f"**🛡️ 安全認證與機制**\n\n{cert_list}")

with tabs[3]: # 商務對接
    st.subheader("✉️ 詢價郵件自動生成")
    email_text = f"""主旨：【詢價】TAD-AGE {platform} 電機系統開發需求\n內容：載重 {weight}kg，目標爬坡 {slope}%，需支援 {v_bus}V 系統。"""
    st.code(email_text, language="markdown")

st.caption(f"TAD-AGE Framework v2.7 | 載重連動與台幣預算優化版")